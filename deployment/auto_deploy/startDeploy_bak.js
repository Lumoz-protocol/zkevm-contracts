        ...
        try {
            ...
            if (regisResponse.rowCount > 0) {
                for (let i = 0; i < regisResponse.rowCount; i++) {
                    ...
                    const cmd = `${depoly_zkrollup} -c ${depoly_zkrollup_config} --id ${regisResponse.rows[i].reg_id} --poe_addr ${outputJson.polygonZkEVMAddress} --exit_mana_addr ${outputJson.polygonZkEVMGlobalExitRootAddress} --gen_blocknumber ${outputJson.deploymentBlockNumber} --sequencer ${sequencer.address}  --aggregator ${aggregator.address} --bridge_addr ${outputJson.polygonZkEVMBridgeAddress} --l2bridge_addrs ${bridgeAddrL2}`;
                    console.log('cmd: ', cmd)
                    await spawn(cmd);
